...
AWeapon::AWeapon()
{
	...
	// 与Replicated，GetLifetimeReplicatedProps一起使用才有效果
	bReplicates = true;
	...
	// 当我们丢下武器时，碰撞体的设置为下面2行
	// 如果已将另一个物理形体设置为"阻挡（Block)，将发生撞击事件；与所有物体发生碰撞
	WeaponMesh->SetCollisionResponseToAllChannels(ECollisionResponse::ECR_Block);
	// 忽略与玩家的物理碰撞,任何由玩家控制的实体的类型都应为Pawn
	WeaponMesh->SetCollisionResponseToChannel(ECollisionChannel::ECC_Pawn, ECollisionResponse::ECR_Ignore);
	
	// 刚开始不启用碰撞体
	WeaponMesh->SetCollisionEnabled(ECollisionEnabled::NoCollision);

	// 设置Mesh的RenderCustomDepth
...
void AWeapon::BeginPlay()
{
	...
	// 如果已将另一个物理形体设置为"重叠（Overlap）"或"阻挡（Block）"，则将发生重叠事件
	// 与玩家互相重叠
...
// AddAmmo只在服务器执行，所以可以直接调用ClientAddAmmo
void AWeapon::AddAmmo(int32 AmmoToAdd)
{
	Ammo = FMath::Clamp(Ammo + AmmoToAdd, 0, MagCapacity);
	// 如果刚好是服务器，那么它需要更新HUD。如果是客户端换弹，那么这一步其实没有意义，因为服务器看不见客户端的HUD
	SetHUDAmmo();

...
void ULagCompensationComponent::BeginPlay()
{
	...
	//FFramePackage Packge;
	//SaveFramePackage(Packge);
	//ShowFramePackage(Packge, FColor::Orange);
}

void ULagCompensationComponent::ServerScoreRequest_Implementation(ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize& HitLocation, float HitTime, AWeapon* DamageCursor)
{
	FServerSideRewindResult Confirm = ServerSideRewind(HitCharacter, TraceStart, HitLocation, HitTime);

	if (Character && DamageCursor && HitCharacter && Confirm.bHitConfirmed)
	{
		UGameplayStatics::ApplyDamage(
			HitCharacter,
			DamageCursor->GetDamage(),
...
void ULagCompensationComponent::ShowFramePackage(const FFramePackage& Packge, const FColor& Color)
{
	for (auto& BoxInfo : Packge.HitBoxInfo)
	{
		DrawDebugBox(
			GetWorld(),
